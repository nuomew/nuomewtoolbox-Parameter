...
namespace NyameauToolbox.Editor
{
    public class VRChatAvatarAnalyzer : EditorWindow
    {
        ...
        private void ValidateDecryption()
        {
            try
            {
                ...
                if (testResult != "测试")
                {
                    ...
                    decryptionErrorMessage = "解密验证失败：测试字符串解密结果不匹配";
                    Debug.LogError("[诺喵工具箱] 解密功能验证失败，插件功能已禁用。");
                }
                else
                {
            ...
            {
                isDecryptionValid = false;
                decryptionErrorMessage = $"解密验证异常: {ex.Message}";
                Debug.LogError($"[诺喵工具箱] 解密功能验证异常，插件功能已禁用。错误: {ex.Message}");
            }
        }
        ...
        private string DecryptContent(string encryptedContent)
        {
            ...
            {
                isDecryptionValid = false;
                decryptionErrorMessage = $"解密失败: {ex.Message}";
                Debug.LogError($"[诺喵工具箱] 内容解密失败，插件功能将被禁用。错误信息: {ex.Message}");
                return "内容解析失败 - 插件功能已禁用";
            }
        }
        ...
        private void DrawDecryptionErrorMessage()
        {
            ...
            GUILayout.Label("⚠️ 插件功能已禁用 ⚠️", errorTitleStyle);
            ...
            GUILayout.Label("内容解密验证失败，插件无法正常使用。", errorDetailStyle);
            ...
            GUILayout.Label("请检查插件完整性或联系开发者获取支持。", solutionStyle);
            
            EditorGUILayout.Space(10);
            
            // 重试按钮
            if (GUILayout.Button("重新验证", GUILayout.Height(30)))
            {
                ...
                if (!isDecryptionValid)
                {
                    Debug.LogError("[诺喵工具箱] 重新验证失败，插件功能仍然被禁用。");
                }
                else
                {
                    Debug.Log("[诺喵工具箱] 重新验证成功，插件功能已恢复。");
                }
            }
